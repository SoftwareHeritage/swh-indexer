...
* The :prop:`as:followers` property lists actors (typically persons) subscribed
  ...
  the number of followers, which matches ``watchers_count`` on GitHub, GitLab;
...
* :prop:`forge:forkedFrom` and :prop:`forge:forks` properties to describe relationships
  between repositories of distributed VCS. (:prop:`forge:forks` is also an
  :prop:`as:OrderedCollection`).
* :prop:`forge:mirrors`, :prop:`forge:mirroredBy`, :prop:`forge:mirrorsTo`, and
  :prop:`forge:mirroredFrom` to describe repositories (typically on different platform)
  whose content are synchronized together.

New relation properties
^^^^^^^^^^^^^^^^^^^^^^^

* The :prop:`mesocore:openIssues` property lists issues in a repository's associated
  issue tracker.
  As it is an :prop:`as:OrderedCollection`, its :prop:`as:totalItems` attribute contains
  the number of followers, which matches ``open_issues_count`` on GitHub and GitLab;
  which can also be expressed with :prop:`schema:InteractionCounter`
